...
def build_warmup_sche(opti, config):
    if config.get('warmup_epoch') is not None:
        scheduler_steplr = torch.optim.lr_scheduler.StepLR(opti, step_size=1, gamma=config.lr_decay ** (1 / config.decay_step))
        scheduler = GradualWarmupScheduler(opti, multiplier=1, total_epoch=config.warmup_epoch,
